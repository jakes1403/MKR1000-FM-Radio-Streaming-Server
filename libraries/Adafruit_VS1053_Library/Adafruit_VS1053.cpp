...
uint16_t Adafruit_VS1053::loadPluginFromMemory(const uint8_t *plugin,
                                               size_t pluginSize) {
  size_t pos = 0;
  ...
  // Check header: 'P' '&' 'H'
  if (plugin[pos++] != 'P' || plugin[pos++] != '&' || plugin[pos++] != 'H') {
    return 0xFFFF;
  }

  uint16_t type;

  while (pos < pluginSize) {
    uint16_t offsets[] = {0x8000U, 0x0000U, 0x4000U};
    ...
    // Need 4 bytes for len + addr
    if (pos + 4 > pluginSize) {
      return 0xFFFF;
    }

    // Read len (big-endian, force even)
    uint16_t len = ((uint16_t)plugin[pos++] << 8);
    len |= ((uint16_t)plugin[pos++] & ~1);

    // Read addr (big-endian)
    uint16_t addr = ((uint16_t)plugin[pos++] << 8);
    addr |= plugin[pos++];

    if (type == 3) {
      // "execute rec" — return address
      return addr;
    }

    // Make sure we have 'len' bytes left for data
    if (pos + len > pluginSize) {
      return 0xFFFF;
    }
    ...
    while (remaining) {
      uint16_t data = ((uint16_t)plugin[pos++] << 8);
      data |= plugin[pos++];
      sciWrite(VS1053_REG_WRAM, data);
      remaining -= 2;
    }
  ...
  // If we fall out of the loop without type==3, treat as failure
  return 0xFFFF;
}

boolean Adafruit_VS1053::prepareRecordOgg_plugin_from_memory(const uint8_t *plugin, size_t pluginSize) {
  sciWrite(VS1053_REG_CLOCKF, 0xC000); // set max clock
  delay(1);
  while (!readyForData())
    ;
  ...
  int pluginStartAddr = loadPluginFromMemory(plugin, pluginSize);
  if (pluginStartAddr == 0xFFFF)
    return false;
  Serial.print("Plugin at $");
  Serial.println(pluginStartAddr, HEX);
